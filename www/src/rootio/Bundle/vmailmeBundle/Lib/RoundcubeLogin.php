...
namespace rootio\Bundle\vmailmeBundle\Lib;
...
class RoundcubeLogin {
     ...
     */
    public function __construct($request, $webmailPath, $enableDebug = false) {
    ...
    private function updateLoginStatus($forceUpdate = false) {
        ...
        if ($this->cookies->has('roundcube_sessid'))
            $this->rcSessionID = $this->cookies->get('roundcube_sessid');

        if ($this->cookies->has('roundcube_sessauth'))
            $this->rcSessionAuth = $this->cookies->get('roundcube_sessauth');
    ...
    private function sendRequest($path, $postData = false) {
        ...
        if (!$this->cookies->get('roundcube_sessid') && $this->rcSessionID)
            $COOKIES[] = "roundcube_sessid={$this->rcSessionID}";

        if (!$this->cookies->get('roundcube_sessauth') && $this->rcSessionAuth)
